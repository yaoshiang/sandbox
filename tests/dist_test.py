...
class DTensorTest(DistributedTestBase):
    ...
    def test_big_tensor_creation_with_meta_device(self):
        """Test creating a large DTensor bigger than any rank's HBM."""
        # Arrange
        self.create_pg("cuda:0")
        device = f"cuda:{self.rank}"
        dtype = torch.bfloat16
        ...
        local_items = local_hbm_bytes // dtype.itemsize
        global_items = local_items * 7 // 4
        global_tensor_size_bytes = global_items * dtype.itemsize

        # Create tensor on meta device
        big_meta_tensor = torch.arange(global_items, device="meta", dtype=dtype)
